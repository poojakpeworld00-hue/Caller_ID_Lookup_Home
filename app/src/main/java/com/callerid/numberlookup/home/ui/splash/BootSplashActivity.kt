package com.callerid.numberlookup.home.ui.splash
...
import com.callerid.adbridge.presentation.showAppRedirectPopup
...
import com.callerid.numberlookup.home.databinding.ActivitySplashBinding
...
import com.callerid.numberlookup.home.ui.intro.IntroRevealPolicy
...
import com.callerid.numberlookup.home.ui.terms.AgreementActivity
 ...
 */
class BootSplashActivity : ScreenBaseActivity<ActivitySplashBinding>() {
    ...
    private companion object {
        // Shared with AdRelayActivity's tracing. Filter: `adb logcat -s SplashFlow`
    ...
    override fun initView() {
        ...
        getData(this, true, object : OnDataReady {
            override fun onSuccess() {
                Log.d(SPLASH_FLOW_TAG, "getData onSuccess → dataReady")
                AppOpenAdRegistry.loadAd(this@BootSplashActivity)
                dataReady.set(true)
                maybeProceed()
            }
...
//        }, WATCHDOG_TIMEOUT_MS)
    ...
    private fun proceedNow() {
        ...
        val redirectLink = AdsVault.getInstance(this).getString("In_App_Update_Link")
    ...
    private fun launchNext() {
        ...
        // AppOpenAdRegistry (via LookupShellApp.handleAppForeground) and,
        // because we've navigated off the excluded BootSplashActivity by then, would
     ...
     *
     * Language, Terms and Onboarding are all gated by [IntroRevealPolicy] (the
    ...
    private fun nextScreen(): Class<*> = when {
        ...
        IntroRevealPolicy.shouldShowLanguage(this) -> LanguagePickActivity::class.java
        IntroRevealPolicy.shouldShowTerms(this) -> AgreementActivity::class.java
        IntroRevealPolicy.shouldShowOnboarding(this) -> TourActivity::class.java
        else -> LauncherFlow.homeActivity()
    }
    private fun route() {
        val next = when {
            !prefs.isLanguageSelected -> LanguagePickActivity::class.java
            !prefs.isTermsAccepted -> AgreementActivity::class.java
            !prefs.isOnboardingDone -> TourActivity::class.java
            else -> AppHubActivity::class.java
        }
